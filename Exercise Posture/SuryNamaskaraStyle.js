...
let currPoseCount = 0;
let suryNamaskaraStyle = 0;
// suryNamaskaraStyle points to which we need to take.
...
function setup() {
    ...
    const modelInfo1 = { // this is squarts posture.
        model: `Models/model.json`, metadata: `Models/model_meta.json`, weights: `Models/model.weights.bin`,
    };
    model1.load(modelInfo1, modelLoaded); // this loads the squarts posture.

    const folderName2 = 2;
    const modelInfo2 = {// this is for tree posture.
        model: `Models/model(1).json`, metadata: `Models/model_meta(1).json`, weights: `Models/model.weights(1).bin`,
    };
...
function gotResult(error, results) {
    ...
    if (results[0].confidence > 0.70) {
        ...
        if (currentPos) poseInfoDiv.html(`Current Pose: ${currentLabel}<br>Pose Timer: ${currPoseCount}`);
        setTimeout(() => {
            isClassifying = true;
            classifyPose();
        }, 1000);
